...
test('reporter factory should return a function', t => {
  t.plan(1);
  t.equal(typeof reporterFactory(), 'function', 'reporter factory has returned a function');
});
...
test('reporter should return a promise', t => {
  ...
  t.equal(typeof reporter({}).then, 'function', 'reporter is then-able');
});
...
test('reporter should NOT write to console if console param is false', t => {
  ...
  t.false(fancyLog.info.called, 'reporter has NOT written anything to console');
  fancyLog.info.restore();
});
